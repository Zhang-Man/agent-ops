...
description: Release and publish the agent-ops standalone remote-ops plugin — bump all packages to one unified version, commit and tag, push the vX.Y.Z tag that triggers the GitHub Actions publish pipeline, and verify the npm publish. Covers post-release verification and bad-version recovery. Use when the user asks to 发布/发版/release/bump 版本/publish the @zhangman2235/agent-ops package.
whenToUse: The user wants to release agent-ops (发布新版、发个版本、release、tag、publish @zhangman2235/agent-ops), build or change the release pipeline (release 管线、CI 发布), or recover from a bad published version (坏包、回滚、deprecate). Not for routine commits or code changes.
---
...
- 仓库：agent-ops（PUBLIC），发布 scope `@linxin666`，registry 固定
  registry.npmjs.org。
- 1 个发布包：packages/agent-ops（独立远程运维插件，SSH + Telnet 双引擎）。
...
2. 全量 gate：typecheck / build / test / test:scripts；
...
```sh
npm view @zhangman2235/agent-ops version   # 期望 = X.Y.Z
