/**
 * The /api/agent-ops route family: host CRUD, exec, cluster, SFTP transfer
...
import type { SshEngine, ShellSession } from './ssh-engine.ts'
import { SSH_API, type HostPayload, type TerminalClientFrame, type TerminalServerFrame } from './ssh-protocol.ts'
...
/**
 * Build every /api/agent-ops route (exact paths) plus the terminal upgrade.
...
export function makeRoutes(deps: SshRoutesDeps): { routes: WebRoute[]; upgrade: WebUpgradeRoute } {
  ...
  const staging = deps.stagingDir ?? join(tmpdir(), 'agent-ops-uploads')
