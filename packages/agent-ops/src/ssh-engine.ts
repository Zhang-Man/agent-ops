...
import type { ClusterResult, ExecResult, SshHostEntry, SshHostSummary, TestResult, TransferProgress, TunnelInfo } from './ssh-protocol.ts'
import { expandHome, type HostStore } from './ssh-store.ts'
...
export class SshEngine {
  ...
  async ls(alias: string, path: string): Promise<import('./ssh-protocol.ts').RemoteDirEntry[]> {
    return this.withClient(alias, (client) => this.withSftp(client, async (sftp) => {
      return await new Promise((resolve, reject) => {
        sftp.readdir(path, (error, list) => {
          ...
          const entries: import('./ssh-protocol.ts').RemoteDirEntry[] = list.map(item => ({
            name: item.filename,
