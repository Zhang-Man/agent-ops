...
export const ENTRY_SELECTOR = '[data-agent-ops-entry]'
...
function placeEntry(root: HTMLElement, entry: HTMLButtonElement): boolean {
  ...
  if (entry.parentElement !== root) {
    ...
    const family = Array.from(root.children).filter(
      (el): el is HTMLElement => el instanceof HTMLElement && el.matches('[data-dsh-taskboard-entry], [data-agent-ops-entry]'),
    )
