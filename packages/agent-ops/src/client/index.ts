/**
 * Browser-half entry for the agent-ops plugin — runs inside the dsh web GUI.
 *
 * Applies the plugin's own DOM-hook shim, registers the agent-ops locale
 * dictionaries, and mounts the two DOM surfaces: the sidebar entry row
 * (toggles the panel) and the operations panel in the center column.
...
/**
 * Mount the shim, locale dictionaries, sidebar entry, and panel.
 * @param ctx - client root context (locale service).
 */
export function apply(ctx: ClientContext): void {
  ctx.effect(() => installShim(), 'agent-ops: dom shim')
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'agent-ops: dictionaries')
  ...
  try {
    ...
    console.warn('[agent-ops] mount failed:', error)
  }
  ctx.effect(() => () => {
