...
import type { ClusterResult, ExecResult, SshHostSummary, TunnelInfo } from './ssh-protocol.ts'
...
/** The host-registration tool: chat-given credentials become a store entry. */
export function sshAddTool(store: HostStore) {
  return defineTool({
    name: 'ssh_add',
    description: 'Register a new SSH host from credentials given in chat (IP/hostname + user + password, or a private-key path). Creates the alias for later ssh_exec / ssh_upload / ... calls. Secrets are stored in ~/.dsh/dsh-ssh.json (mode 0600). ' +
      'Triggers: add server, register host, save server credentials, connect a new machine.',
    parameters: {
      ...
      user: { type: 'string', required: true, description: 'Login user.' },
      port: { type: 'integer', description: 'SSH port (default 22).' },
      password: { type: 'string', description: 'Password auth; omit to use key auth.' },
      keyPath: { type: 'string', description: 'Absolute private-key path; required when password is omitted.' },
      passphrase: { type: 'string', description: 'Passphrase for an encrypted key.' },
      description: { type: 'string' },
    ...
    output: {
      ...
      render: (_args, value: { ok: boolean; alias?: string; host?: string; port?: number; user?: string; auth?: string; error?: string }) => {
        if (!value.ok) return text('ssh_add failed: ' + (value.error ?? 'unknown error'))
        return text(`host '${value.alias}' added: ${value.host}:${value.port} as ${value.user} (auth: ${value.auth}). Stored in ~/.dsh/dsh-ssh.json (mode 0600). Verify with ssh_exec ${value.alias} "uname -a".`)
      },
    },
    async execute(args) {
      try {
        const hasPassword = args.password !== undefined && args.password !== ''
        const hasKey = args.keyPath !== undefined && args.keyPath !== ''
        if (!hasPassword && !hasKey) throw new Error('password or keyPath is required')
        const entry = store.create({
          ...
          auth: hasPassword
            ? { kind: 'password', password: args.password }
            : { kind: 'key', keyPath: args.keyPath, ...(args.passphrase !== undefined && args.passphrase !== '' ? { passphrase: args.passphrase } : {}) },
          ...(args.description !== undefined ? { description: args.description } : {}),
          ...(args.environment !== undefined ? { environment: args.environment } : {}),
          tags: args.tags ?? [],
        })
        return { ok: true, alias: entry.alias, host: entry.host, port: entry.port, user: entry.user, auth: entry.auth.kind }
      } catch (error) {
        return { ok: false, error: error instanceof Error ? error.message : String(error) }
      }
    },
