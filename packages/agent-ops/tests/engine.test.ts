...
/**
 * The real-sshd SFTP case needs /usr/sbin/sshd plus a real system user in
 * process.env.USER (its AllowUsers line). Environments without either skip
 * the block instead of failing (e.g. DSH child processes whose USER is a
 * pseudo-user); the embedded ssh2 server covers the SFTP surface elsewhere.
 */
const realSshdReady = existsSync('/usr/sbin/sshd')
  && process.env.USER !== undefined
  && new RegExp(`^${process.env.USER}:`, 'm').test(readFileSync('/etc/passwd', 'utf8'))
...
describe.skipIf(!realSshdReady)('sftp (real sshd)', () => {
