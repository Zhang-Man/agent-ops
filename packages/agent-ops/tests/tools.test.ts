...
describe('tool factories (defineTool DSL regression)', () => {
  it('constructs every tool without throwing', () => {
    ...
    expect(() => sshAddTool(new HostStore(join(tmpdir(), `dsh-ssh-ctor-${Date.now()}`, 'dsh-ssh.json')))).not.toThrow()
  })
})
...
describe('ssh_add', () => {
  it('registers a password host from chat credentials without echoing the secret', async () => {
    const store = new HostStore(join(tmpdir(), `dsh-ssh-add-${Date.now()}`, 'dsh-ssh.json'))
    const tool = sshAddTool(store)
    const result = await run(tool, { alias: 'db-01', host: '10.0.0.9', user: 'root', password: 's3cret' })
    expect(result.ok).toBe(true)
    expect(result.auth).toBe('password')
    const text = render(tool, result)
    expect(text).toContain('db-01')
    expect(text).not.toContain('s3cret')
    expect(store.find('db-01')?.auth.password).toBe('s3cret')
  })

  it('requires password or keyPath', async () => {
    const store = new HostStore(join(tmpdir(), `dsh-ssh-add-${Date.now()}`, 'dsh-ssh.json'))
    const tool = sshAddTool(store)
    const result = await run(tool, { alias: 'db-01', host: '10.0.0.9', user: 'root' })
    expect(result.ok).toBe(false)
    expect(result.error).toContain('password or keyPath is required')
  })

  it('registers a key host and never stores a password', () => {
    const store = new HostStore(join(tmpdir(), `dsh-ssh-add-${Date.now()}`, 'dsh-ssh.json'))
    const tool = sshAddTool(store)
    void run(tool, { alias: 'k8s-01', host: '10.0.0.8', user: 'core', keyPath: '/root/.ssh/id_ed25519', passphrase: 'kp' })
    const entry = store.find('k8s-01')
    expect(entry?.auth.kind).toBe('key')
    expect(entry?.auth.password).toBeUndefined()
    expect(entry?.auth.passphrase).toBe('kp')
  })
})
