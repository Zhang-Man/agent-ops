...
| Agent 工具 | `ssh_add` / `ssh_list` / `ssh_exec` / `ssh_upload` / `ssh_download` / `ssh_tunnel` / `ssh_cluster`，GUI 与 Agent 共享同一份主机配置 |
...
- 隧道只监听 `127.0.0.1`。
- Agent 操作的主机来源：在 GUI 中配置、从 ~/.ssh/config 导入，或用 `ssh_add` 按用户在对话中明确给出的凭据（IP + 用户名 + 密码，或密钥路径）注册；对话中给出的密码会同时落入 0600 配置文件与会话记录，敏感环境优先密钥或 GUI 导入。
