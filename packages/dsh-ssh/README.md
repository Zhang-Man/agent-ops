...
| Agent tools | `ssh_add` / `ssh_list` / `ssh_exec` / `ssh_upload` / `ssh_download` / `ssh_tunnel` / `ssh_cluster`; GUI and Agent share the same host config |
...
- Hosts the Agent operates on come from: the GUI, an import from ~/.ssh/config, or `ssh_add` with credentials the user explicitly gave in chat (IP + user + password, or a key path); chat-given passwords land in both the 0600 config file and the conversation log — prefer keys or the GUI import for sensitive environments.
