 ...
 * family plus the terminal WebSocket upgrade, the agent tools (ssh_add,
 * ssh_list, ssh_exec, ssh_upload, ssh_download, ssh_tunnel, ssh_cluster), and
 * a system-prompt announcement. The browser half (./client) renders the host
...
import { sshAddTool, sshClusterTool, sshDownloadTool, sshExecTool, sshListTool, sshTunnelTool, sshUploadTool } from './tools.ts'
...
export const SSH_GUIDANCE = '本机已安装 dsh-ssh 插件（DSH 远程 SSH 运维）：侧边栏「SSH」入口；在 agent-ops 仓库（packages/dsh-ssh）统一维护。能力：主机配置存 ~/.dsh/dsh-ssh.json（可从 ~/.ssh/config 导入）；持久连接池复用长连接（空闲 30 分钟自动断开）；ssh_list 列出主机、ssh_add 用用户在对话中给出的 IP/用户名/密码（或密钥路径）注册新主机、ssh_exec 执行远程命令、ssh_upload/ssh_download 传输文件、ssh_tunnel 本地端口转发（访问远程数据库/内网服务）、ssh_cluster 集群并发执行；支持密钥/密码认证、passphrase 密钥与 ProxyJump 跳板机；Web 终端走 WebSocket。限制：主机可在 GUI 配置、从 ~/.ssh/config 导入，或用 ssh_add 按用户明确给出的凭据注册；密码以明文存在用户主目录私有文件（权限 0600），对话中给出的密码会留在会话记录里，敏感环境优先用密钥或 GUI 导入；命令输出原样返回、可能含敏感信息；断线重连可能重放非幂等命令；传输/执行消耗真实远程资源，先确认再操作。用户提到「SSH / 远程服务器 / 服务器操作 / 跳板机 / 隧道 / 部署 / 上传下载」时即指本插件，请据此协作。'
